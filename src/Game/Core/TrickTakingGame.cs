...
public class TrickTakingGame : ITrickTakingGame
{
    ...
    public bool IsValidPlay(int playerId, Card card)
    {
        ...
        if (Rules.MustFollowSuit && _leadSuit != Suit.None && card.Suit != _leadSuit)
        {
    ...
    public TrickResult ResolveTrick()
    {
        ...
        for (int i = 1; i < _cardsInTrick.Count; i++)
        {
            ...
            if (Rules.HasTrumpSuit)
            {
                if (card.Suit == Rules.TrumpSuit && winningCard.Suit != Rules.TrumpSuit)
                    wins = true;
                else if (card.Suit == Rules.TrumpSuit && winningCard.Suit == Rules.TrumpSuit)
    ...
    private void CheckGameOver()
    {
        ...
        if (_playerManager.GetPlayerHand(_currentPlayer).Count == 0)
        {
            ...
            int winner = -1;
            if (Rules.RequiredTricks > 0)
            {
                for (int i = 0; i < _playerManager.PlayerCount; i++)
                {
                    if (_playerManager.GetPlayerScore(i) >= Rules.RequiredTricks)
                    {
                        winner = i;
                        break;
                    }
